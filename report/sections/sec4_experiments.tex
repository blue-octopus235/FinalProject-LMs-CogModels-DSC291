...
\end{itemize}
H3 is evaluated on the matched subset (Section~\ref{sec:metrics}). Because only
three seeds are available, we test the per-condition gap difference
$\Delta_{\text{LSTM}} - \Delta_{\text{RNNG}}$ with a pair-level bootstrap
(resampling the matched pairs, $10^{4}$ replicates) for a 95\% confidence
interval and a one-sided $p$-value, and report the seed-level mean $\pm$ std as
descriptive context.
