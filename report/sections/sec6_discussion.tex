...
act. Up to $10\%$ noise, learning is merely \emph{delayed}, with every condition
converging to the same accuracy given enough steps, whereas at $50\%$ it is
...
withstands noise better than a purely sequential one (LSTM), is now answered in
the affirmative by the RNNG sweep (Section~\ref{sec:rnng}). At matched noise the
RNNG is more accurate than the LSTM throughout, and the advantage \emph{grows}
with noise, from under two points on clean data to $12.6$ points at $50\%$
...
whereas the RNNG's gap stays low and almost flat ($0.020 \to 0.036$), and under
the heaviest noise it is still only half the LSTM's clean-data gap. An explicit
...
Three caveats bound these claims, and we treat them fully in
